...
object IOUtil {
  ...
  /** Copies from one stream to other */
  def copy(in: InputStream, out: OutputStream, bufferSize: Int = 1024 * 4): Unit = {
  ...
  /** Loads java properties file into Scala map */
  ...
  def writeToFile(f: File, content: String): Unit = {
