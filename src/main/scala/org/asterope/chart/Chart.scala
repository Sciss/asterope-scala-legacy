...
case class Chart(
  ...
  def projectLine(line: SkyLine): Option[Path2D] = {
    ...
    var oldPos  = Option.empty[Point2d]
    for {
      vect <- line.skyLineIterator(pixelAngularSize * 3) //TODO distance for line should be adaptive
  ...
  /**
   * Tests if given vector can be projected into canvas
   */
  ...
  /**
   * Tests if given rectangle is inside or intersects with canvas
   */
  ...
  /**
   * Tests if given node is inside or intersects with canvas
   */
   ...
   *
   * @param layer where node will be added
   ...
   */
  def addNode(layer: Layer.Value, node: PNode, obj: Any = None, zorder: Double = 0): Unit =
    exec {
