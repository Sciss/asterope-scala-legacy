...
abstract class ScalaTestCase extends TestCase {
  ...
  implicit class Operators(a1: Any) {
    ...
    def ~==(v2: Double): Option[String] = {
      ...
      if (abs(v1 - v2) < abs(max(v1, 1) / 1e12)) None
  ...
  def fail(): Unit = Assert.fail()

  def fail(msg: String): Unit = Assert.fail(msg)
  ...
  def assert(error: Option[String]): Unit =
    if (error != None) throw new AssertionFailedError(error.get)

  def assert(error: Option[String], msg: String): Unit =
  ...
  def intercept[T <: AnyRef](block: => Any)(implicit manifest: Manifest[T]): Unit =
    try {
      ...
      case u: Throwable =>
        ...
        if (!clazz.isAssignableFrom(u.getClass)) throw u  //wrong type of exception, rethrow
    }
   ...
   *    }
   */
  def expect(value: Any)(block: => Any): Unit = assert(value === block)

  def sleep(i: Long): Unit = Thread.sleep(i)

  /** Waits until conditions became true
    * Periodically query expression, and blocks until it returns true
    * Or throw exception if it takes more than 30 seconds
    */
  def waitUntil(block: => Boolean, msg: String = "Time out!", timeout: Int = 30000): Unit = {
...
class ScalaTestCaseExampleTest
  extends ScalaTestCase {
  //notice class it extends instead of 'TestCase'
