...
object Skyview extends ChartFeature[SkyviewConfig] {
  ...
  def updateChart(chart: Chart, m: SkyviewConfig): Unit =
    if (m.useInternalEngine)
      updateChartInternalEngine(chart, m)
    else
      updateChartOnline(chart, m)

  def updateChartInternalEngine(chart: Chart, m: SkyviewConfig): Unit =
    Skyview.lock.synchronized {
      try {
        ...
        val imager = new Imager(chart.width, chart.height, chart.wcs)
        imager.run()
        ...
        imgp.getProcessor.scale(1d, -1d)
        //for some reasons it is upside down, turn it back
  ...
  protected def updateChartOnline(chart: Chart, m: SkyviewConfig): Unit = {
    ...
    url += "&pixels=" + chart.width + "," + chart.height
