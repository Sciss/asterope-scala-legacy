...
object ChartUtils {
  ...
  def saveChartToImageFile(chart: Chart, file: File) {
    ...
    if (ext == "svg") {
      ...
      g2.setSVGCanvasSize(new Dimension(chart.width, chart.height + chart.legendHeight))
  ...
  /** Iterates over node and all its children */
...
private[chart] class GradientExtensionHandler extends DefaultExtensionHandler {
  override def handlePaint(paint: Paint, generatorCtx: SVGGeneratorContext): SVGPaintDescriptor = paint match {
    case p: RadialGradientPaint =>
    ...
    case _ => super.handlePaint(paint, generatorCtx)
  }
}
