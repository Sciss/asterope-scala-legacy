...
class Rotater(matrix2: RealMatrix) extends Transformer {
  ...
  require(matrix2.getRowDimension == 3 && matrix2.getColumnDimension == 3)
  ...
  /** Get the transpose of the Matrix.  For rotation
    ...
    */
  protected lazy val transpose: Rotater = new Rotater(matrix.transpose)
...
object Rotater {
  ...
  def apply(order: String, ang1: Double, ang2: Double, ang3: Double) = {
    ...
    assert(order.length <= 3, "too long order string")
    for {
      i <- 0 until order.length
      axis = order(i)
    } {
