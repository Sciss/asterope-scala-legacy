...
object GetURL {
  ...
  val cacheFolder = new File("netcache")  // TODO hardcoded cacheFolder
...
cat2_local = file:///XMPCC"""
  ...
  /** Loads URL overrides from properties file file */
  def loadUrlOverrides(p: Map[String, String]): Unit =
    for {
  ...
  /** Remote URLs can be downloaded by user and stored locally.
    ...
    */
  def addFolderOverride(prefix: URL, localFolder: URL): Unit =
    urlOverride += prefix -> localFolder  //make sure it has protocol prefix
