...
class Stars(dao: LiteStarDao)
  ...
  /**
	 ...
	 * 
	 * @param chart the chart into which to paint
	 * @param star  the star to paint
  ...
  def paintObject(chart: Chart, config: StarsConfig, star: LiteStar, addToLayer: Boolean): Option[PNode] = {
    ...
    if (config.showVariable && star.minMag.isDefined &&
      star.mag.mag + 3 < limitStarMag.mag) {
      //TODO hardcoded limit not to show variables by brightness
  ...
  def updateChart(chart: Chart, config: StarsConfig): Unit = {
    val limitStarMag = calculateLimitStarMag(chart, config)
    var counter = 0
    val stars = dao.starsByAreaMag(chart.area, limitStarMag).map { s => counter += 1; s } // XXX TODO use stars.size
