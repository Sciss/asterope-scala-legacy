...
class LegendBorder(stars: Stars, deepSky: DeepSkyPainter)
  ...
  /** Updates chart with this feature.
	  ...
	  */
  def updateChart(chart: Chart, config: LegendBorderConfig): Unit = {
    ...
    def append(node: PNode, l: String): Unit = {
    ...
    chart.exec {
      //can not use `addNode`, legend is outside of canvas
