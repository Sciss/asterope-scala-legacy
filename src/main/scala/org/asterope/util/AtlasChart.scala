...
object AtlasChart {
  ...
  def millenniumAtlas(ra2: Double, dec2: Double): Int = {
    ...
    if (dec >= 87.0)
      ...
      val gore      = (ra / 8.0).toInt
      var zone      = ((93.0 - dec) / 6.0).toInt
      val remains   = math.ceil(ra / 8.0) * 8.0 - ra
      val per_zone  = Array(2, 4, 8, 10, 12, 14, 16, 20, 20, 22, 22, 24, 24, 24, 24, 24, 24, 24, 24, 24, 22, 22, 20, 20, 16, 14, 12, 10, 8, 4, 2)
      page = (remains * per_zone(zone).toDouble / 8.0).toInt + 1 + gore * 516
      while (0 != {
        zone -= 1
        zone
      }) page += per_zone(zone)
  ...
  def uranometria(ra2: Double, dec2: Double, fix472: Boolean): Int = {
    ...
    var page = angle.toInt % uranometriaNDivides(divide) + startValue
  ...
  def rukl(lon: Double, lat: Double): String = {
    ...
    val z     = math.sin(lat)
    var page  = -1
    var ix    = (y * 5.5 + 5.5).toInt
    val iy    = (4.0 - 4.0 * z).toInt
    val page_starts = Array(-1, 7, 17, 28, 39, 50, 60, 68)
    val buff  = new StringBuffer("Rukl ")
    if (x > 0.0) {
      if (iy <= 1 || iy >= 6) {
        if (0 == ix) ix = 1
        else if (10 == ix) ix = 9
    ...
    if (x < math.Pi * 8.0 / 180.0 && x > -math.Pi * 8.0 / 180.0) {
      val zone_no = (math.atan2(z, y) * 4.0 / math.Pi + 4.0).toInt
