...
case class Scaler(x0: Double, y0: Double, a00: Double, a01: Double, a10: Double, a11: Double) extends Distorter {
  ...
  def getParams: Array[Double] = Array[Double](x0, y0, a00, a01, a10, a11)

  /** Scales a single point where the user supplies the output.
  ...
  def isInverse(trans: Transformer): Boolean = {
    ...
    val sum: Scaler = add(trans.asInstanceOf[Scaler])
    sum.isUnit
  }
  ...
  /** What is the scale of this transformation? This is defined as
    ...
    */
  def scale: Double =
    math.sqrt((a00 + a01) * (a00 + a01) + (a10 + a11) * (a10 + a11)) / math.sqrt(2)
  ...
  private def isUnit: Boolean =
    (abs(x0) + abs(y0) + abs(a01) + abs(a10) + abs(1 - a00) + abs(1 - a11)) < 1.0e-10
}
