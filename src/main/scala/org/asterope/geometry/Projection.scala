...
class Projection {
  ...
  /** Gets the rotation that needs to be performed before the rotation. */
  def getRotater: Rotater = rotation

  /** Updates the Rotater... */
  def setRotater(rot: Rotater): Unit = rotation = rot

  /** Gets the projection algorithm associated with this rotation. */
  def getProjecter: Projecter = proj

  /** Gets any distortion in the plane associated with this projection. */
  def getDistorter: Distorter = dist

  protected def setDistorter(dist: Distorter): Unit =
    this.dist = dist

  protected def specialReference: Array[Double] = null
  ...
  def isFixedProjection: Boolean = fixedProjection

  /** Gets the current reference position */
  def getReferencePoint: Array[Double] = refProj

  /** Creates the specified projection.
    * @param typ	   The three character string defining
    *                 the projection.
  ...
  def this(typ: String, reference: Array[Double]) {
    ...
    if (specialReference != null) {
      val spec: Array[Double] = specialReference
  ...
  /** Unit vector for standard reference point for this
    ...
    */
  private var refProj                     = Array[Double](0, math.Pi / 2)
