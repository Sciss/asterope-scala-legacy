...
class ChartEditor(
    ...
    camera.addInputEventListener(new PBasicInputEventHandler {
      override def mouseClicked(event: PInputEvent): Unit =
        ...
        if (event.isMouseEvent && event.getClickCount == 1 && event.getButton == 1) {
          ...
          overview.updatePointer(chartBase, lowQuality = true)
          refresh()
        }
  ...
  addComponentListener(new ComponentAdapter {
    override def componentResized(e: ComponentEvent): Unit = refresh()
  })
  ...
  protected var allSkyConfig      = Option.empty[AllSkySurveyMem]
  ...
  def waitForRefresh(): Unit =
    refreshLock.synchronized {  //wait until refresh starts
    ...
    refreshWorker = future[Unit] {
      refreshLock.synchronized {
        try {
          ...
          case _: InterruptedException => // can be ignored
          case e: Throwable =>
  ...
  private var selectedPointer = Option.empty[PNode]
  private var selectedObject  = Option.empty[Any  ]

  def getSelectedObject = selectedObject

  val onSelectionChanged = new Publisher[Chart]()

  def selectObject(obj: Option[Any], selectionAfterRefresh: Boolean = false): Unit = {
    ...
    if (obj == selectedObject && !selectionAfterRefresh) return
  ...
  object overview extends PCanvas {
    ...
    def update(detailChart: Chart): Unit = {
      ...
      updatePointer(detailChart, lowQuality = false)
    ...
    //refresh when canvas size changes
    addComponentListener(new ComponentAdapter {
      override def componentResized(e: ComponentEvent): Unit = ChartEditor.this.refresh()
    })

    /** Paints active area on chart */
    def updatePointer(detailChart: Chart, lowQuality: Boolean): Unit = {
  ...
  def showSkyviewSurveyDialog(survey: String): Unit = {
