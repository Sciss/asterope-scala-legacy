...
object Angle {
  ...
  final class AngleWrapperDouble(val d: Double) extends AnyVal {
  ...
  protected[util] implicit def long2angle  (d: Long  ) = new AngleWrapperDouble(d)
  protected[util] implicit def int2angle   (d: Int   ) = new AngleWrapperDouble(d)
  protected[util] implicit def double2angle(d: Double) = new AngleWrapperDouble(d)
  ...
  def randomRa(): Angle = new Angle((CIRCLE * math.random).toInt)
  /** returns random angle with value between -PI/2 and + PI/2 */
  def randomDe(): Angle = new Angle((CIRCLE / 2 * math.random - CIRCLE / 4).toInt)
  ...
  /** Parses Declination from four values. It uses BigDecimal, so there are no rounding errors
    *
  ...
  /** Parses Right ascencion  from triple values raHour raMin, raSec
    * This method uses big decimal, so there are no rounding errors
    *
    * @param raHour  hour value
    * @param raMin   minute value
    * @param raSec   second value
  ...
  def parseRa(ra: String): Angle = {
    ...
    ra match {
      ...
      case r3(d, m, s) => d.toDouble.degree +
                          m.toDouble.arcMinute +
                          m.toDouble.arcSec
  ...
  def parseRaDe(str: String): (Angle, Angle) = {
    ...
    str match {
      ...
      case r4(ra, de) => (ra.toDouble.arcHour, de.toDouble.degree) // XXX TODO: why doesn't implicit kick in?
  ...
  /** Converts RA to string in format '1h 2m'
  ...
  /** Converts DE to string in format '1d 2m'
    * minutes and seconds are auto added as needed
    *
  ...
  /** Calculates great circle distance of two points,
  ...
  val Mas2R = D2R / 3600000.0
  /** multiply to convert micro arc seconds to radians */
  val Uas2R = D2R / 3600000000.0
