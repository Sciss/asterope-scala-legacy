...
package object util {
  ...
  def waitOrInterrupt(futures: Iterable[Future[_]]): Unit =
    try {
  ...
  /** Checks if current thread was interrupted (tasks was cancelled by user).
    * If yes it throws `InterruptedException`
    */
  def checkInterrupted(): Unit =
  ...
  def assertNormalized(v: Vector3D): Unit =
   ...
   */
  def requireEDT(): Unit =
    require(isEDT, "Must be called from Swing event dispatching thread (EDT)")

  /**
   * Throws AssertionError if current thread is NOT Swing event dispatching thread (EDT)
   */
  def requireNotEDT(): Unit =
    require(!isEDT, "Can not be called from Swing event dispatching thread (EDT)")
