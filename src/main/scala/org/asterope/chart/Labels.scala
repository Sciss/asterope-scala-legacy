...
object Labels
  ...
  def updateChart(chart: Chart, config: LabelsConfig): Unit = {
    val objs: List[(HasNomenclature, PNode)] = findStarsToLabel(chart, config) ::: findDeepSkyToLabel(chart, config)

    chart.exec {
      //this all must be on EDT thread, otherwise chart would repaint many times and cause poor performance
      var size: Float = config.labelFont.getSize
      objs.foreach {
        case (starlinkObj, node) =>
          checkInterrupted()
          if (size > 4)
            size = size - 0.03F
  ...
  def findStarsToLabel(chart: Chart, config: LabelsConfig): List[(LiteStar, PNode)] = {
    ...
    val liteStars = chart.objects
      ...
      .filter {
        s => chart.isInsideCanvas(chart.getNodeForObject(s).get)
      }
  ...
  /** Finds best position where to put label. It uses fuzzy algorithm to find
