...
import java.io.{ByteArrayOutputStream, ObjectOutputStream, ObjectInputStream, ByteArrayInputStream, FileOutputStream, IOException, FileInputStream, File}
...
class ConfigStore {
  ...
  def loadProps(fileName: String): Properties = {
	 ...
	 */
  def storeProps(fileName: String, props: Properties): Unit = {
    val f = fileFor(fileName)
  ...
  def propsToCase[E](prefix: String, props: Properties, default: E): E = {
    ...
    for {
      f <- c.asInstanceOf[AnyRef].getClass.getDeclaredFields
  ...
  /**
	 *  Saves case class into file with given name
	 */
  def storeCase(fileName: String, cas: AnyRef): Unit = {
    val props = caseToProps(cas)
    storeProps(fileName, props)
  }

  /**
	 *  Reads case class from file with given name
	 */
