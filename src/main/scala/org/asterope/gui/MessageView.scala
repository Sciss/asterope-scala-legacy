...
class MessageView(resMap: ResourceMap) extends JComponent {
  ...
  object table extends JXTable(data) {
    ...
    object errorPredicate extends HighlightPredicate {
      override def isHighlighted(renderer: Component, adapter: ComponentAdapter): Boolean =
        data(adapter.row).getLevel == Level.SEVERE
    }

    setHighlighters(new ColorHighlighter(errorPredicate, getBackground, Color.RED))
  ...
  protected object MessageViewLogHandler extends Handler {
    ...
    def publish(record: LogRecord): Unit = onEDT {
      data += record
  ...
  protected def getMessage(rec: LogRecord): String =
  ...
  def makeHtml(sel: LogRecord) = {
    ...
    val text = "" +
      entry(resMap.getString("message"  ), sel.getMessage                   , pre = false) +
      entry(resMap.getString("time"     ), new Date(sel.getMillis).toString , pre = false) +
      entry(resMap.getString("severity" ), sel.getLevel.toString            , pre = false) +
      (if (thrown != null) entry(resMap.getString("exception"), thrown, pre = true) else "")
    text
  }

  /** show dialog with details of an LogRecord */
  def showDetail(msg: LogRecord): Unit = {
